...
void affichePlanete(planete p) {
  printf(
      "%s : \ndensite=%.2f\nsa distance au soleil en millions de "
      "kilometres=%.2f\nle nombre de ses satellites=%d\n",
      p.nom, p.densite, p.kilometres, p.satellites);
}
...
void modifieDensite(planete *p, float densite) { p->densite = densite; }
